...
namespace SISFALLA
{
    public partial class FrmAlimentador : ABMBaseForm, IOperacionAlimentador
    {
        ...
        protected override bool Guardar()
        {
            ...
            if (DatosValidos())
            {
                DateTime fechaAperturaAnterior = _opAlimentador.FechaApertura;
                ...
                {
                    if (!_opAlimentador.EsNuevo && fechaAperturaAnterior != _opAlimentador.FechaApertura)
                    {
                        DateTime fechaTemp = _opAlimentador.FechaApertura;
                        _opAlimentador.FechaApertura = fechaAperturaAnterior;
                        ModeloMgr.Instancia.OperacionAlimentadorMgr.Borrar(_opAlimentador);
                        _opAlimentador.FechaApertura = fechaTemp;
                        _opAlimentador.EsNuevo = true;
                    }
        ...
        private bool DatosValidos()
        {
            ...
            {
                if (_txtFechaHoraAp.Value < _regFalla.FecInicio.AddMinutes(deltaTiempo*-1))
                {
