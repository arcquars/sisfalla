...
namespace SISFALLA
{
    public partial class FormCompComprometido : ABMBaseForm, IRRegFallaComponente
    {
        ...
        protected override bool Guardar()
        {
            ...
            if (DatosValidos())
            {
                DateTime fechaAperturaAnterior = _compComprometido.FecApertura;
                ...
                {
                     if (!_compComprometido.EsNuevo && fechaAperturaAnterior != _compComprometido.FecApertura)
                    {
                        DateTime fechaTemp = _compComprometido.FecApertura ;
                        _compComprometido.FecApertura = fechaAperturaAnterior;
                         ModeloMgr.Instancia.RRegFallaComponenteMgr.Borrar (_compComprometido) ;

                        //ModeloMgr.Instancia.OperacionInterruptoresMgr.Borrar(_opInterruptor);
                        _compComprometido.FecApertura  = fechaTemp;
                        _compComprometido.EsNuevo = true;
                        _ctrlTiemposDetalle.SetNuevo();
                        _ctrlAsignacionResponsabilidad.SetNuevo();
                        }
        ...
        private bool DatosValidos()
        {
            ...
            DateTime RegistroFalla = CNDC.BLL.Sesion.Instancia.GetObjetoGlobal<RegFalla>("Principal.FallaActual").FecInicio;
            ...
            {
                if (_txtFechaHoraFalla.Value < RegistroFalla.AddMinutes(deltaTiempo * -1))
                {
                    _errorProvider.SetError(_txtFechaHoraFalla, MessageMgr.Instance.GetMessage("VERIFIQUE_FECHA"));
                    resultado = false;
                }
