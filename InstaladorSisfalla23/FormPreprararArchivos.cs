    ...
    public partial class FormPreprararArchivos : Form
    {
        ...
        private bool CopiarDirectorios()
        {
            ...
            try
            {
                ...
                {
                    Directory.CreateDirectory(dirPath.Replace(ruta, FormInstalador.rutaDestino));
                ...
                foreach (string newPath in Directory.GetFiles(ruta, "*.*",
                    SearchOption.AllDirectories))
                {
                    File.Copy(newPath, newPath.Replace(ruta, FormInstalador.rutaDestino), true);
                    incrementar(5); 
                }
            }
            catch (Exception)
        ...
        private bool CrearDirectorios()
        {
            ...
            try
            {
                ...
                if (!System.IO.Directory.Exists(FormInstalador.rutaDestino))
                {
                    System.IO.Directory.CreateDirectory(FormInstalador.rutaDestino);
                    incrementar(5);
                }
                DirectoryInfo myDirInfo = new DirectoryInfo(FormInstalador.rutaDestino);
        ...
        public void Iniciar()
        {
            ...
            if (!CrearDirectorios())
            {
                string msg = string.Format("No se pudo borrar la carpeta :{0}. Por favor eliminela manualmente.", FormInstalador.rutaDestino);
                MessageBox.Show(msg);
                Application.Exit();
            }
