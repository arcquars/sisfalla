...
namespace CNDC.DAL
{
    public class CommentMgr
    {
        ...
        private void Ensure(string tableName)
        {
            if (!_comments.ContainsKey(tableName))
            {
                OracleCommand command = ConnexionOracleMgr.Instancia.CrearCommand();
                command.CommandText = string.Format("SELECT * FROM {0} WHERE owner ='QUANTUM' AND table_name = :table_name", COMMENTS_TABLE_NAME);
                command.Parameters.Add("table_name", OracleDbType.Varchar2, tableName, ParameterDirection.Input);
                try
                {
                    DataTable tablaComentarios = ConnexionOracleMgr.Instancia.EjecutarCmd(command);
