...
{
    public class RptComponentesAltas : ProveedorDatosBase
    {
        private DateTime _fechaVigencia;
        public RptComponentesAltas()
        {
        ...
        private DataTable GenerarFechas()
        {
         ...
         @"SELECT to_char(a.fecha_v,'dd/MM/yyyy') as FECHA_VIGENCIA, to_char(a.fecha_v,'dd/MM/yyyy') as FECHA_VALOR
            from
            (select distinct (fecha_ingreso) as fecha_v from f_ac_componente
              where not fecha_ingreso is null
              order by 1 DESC
            ) a";
        ...
        private DateTime GetUltimaFecha()
        {
            ...
            from
            (select max(fecha_ingreso) as fecha_v from f_ac_componente
              where not fecha_ingreso is null
            ) a";
            OracleCommand cmd = CrearComando(sql);
            cmd.CommandText = sql;
