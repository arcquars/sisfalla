...
{
    class RptComponenteLineas : ProveedorDatosBase
    {
        public override List<DataTable> GetDatos()
        {
            ...
            ORIGEN.NOMBRE_NODO AS NODO_ORIGEN,
            DESTINO.NOMBRE_NODO AS NODO_DESTINO
            FROM
            F_AC_COMPONENTE
            ...
            LEFT JOIN F_AC_RCOMPONENTE_NODO N_DESTINO ON F_AC_COMPONENTE.PK_COD_COMPONENTE = N_DESTINO.PK_COD_COMPONENTE AND N_DESTINO.D_COD_TIPO_RELACION in (3603) AND N_DESTINO.FECHA_SALIDA is NULL
            LEFT JOIN F_AC_NODO ORIGEN ON N_ORIGEN.PK_COD_NODO = ORIGEN.PK_COD_NODO
            LEFT JOIN F_AC_NODO DESTINO ON N_DESTINO.PK_COD_NODO = DESTINO.PK_COD_NODO
            WHERE
            sysdate between N_ORIGEN.FECHA_INGRESO and NVL(N_ORIGEN.FECHA_SALIDA, sysdate + 1)
