    ...
    class DBIntranet
    {
        ...
        public void Guardar(RegistroPublica registro)
        {
            ...
            try
            {
                if (existe(registro))
                {
                    query = "UPDATE " + registro.tabla + " SET {1}=?{1},{4}=date_format( sysdate( ) , '%H:%i:%s' ),{5}=?{5},{6}=?{6} WHERE {0}=?{0} AND {2}=?{2} AND {3}=?{3}";
                }
                else
                {
                    query = "INSERT INTO " + registro.tabla + " ({0},{1},{2},{3},{4},{5},{6}) VALUES (?{0},?{1},?{2},?{3},date_format( sysdate( ) , '%H:%i:%s' ),?{5},?{6})";
                }
                query = string.Format(query,
                        "idsubmenu",
                        "titulo",
