...
namespace SISFALLA
{
    public partial class EnvioCorreo : ABMBaseForm, IFuncionalidad
    {
        ...
        public DataRow consultaConfiguracion(long CodEmpresa)
        {
            ...
            strSql =
                @"SELECT
                        ...
                        AND A.PK_COD_CONFI=:COD_DOMINIO";

            agregarConsulta(strSql);
            agregarParametro("COD_EMPRESA", CodEmpresa.ToString());
            agregarParametro("COD_DOMINIO", ((int)DOMINIOS_ENVIOCORREO.ENVIOCORREO_PORDEFECTO).ToString());
        ...
        public DataRow consultaConfiguracion(long CodEmpresa, string CodDominio)
        {
            string strSql = string.Empty;
            DataTable t = new DataTable();
            strSql =
                @"SELECT
                        A.*
                        FROM
                        P_DEF_SERVICIOENVIO A, P_DEF_DOMINIOS D
                        WHERE
                        A.PK_COD_PERSONA = :COD_EMPRESA
                        AND
                        D.D_COD_TIPO = :COD_DOMINIO
                        AND 
                        D.COD_DOMINIO = A.PK_COD_CONFI";

            agregarConsulta(strSql);
            agregarParametro("COD_EMPRESA", CodEmpresa.ToString());
            agregarParametro("COD_DOMINIO", CodDominio);
            OracleDataAdapter adap = new OracleDataAdapter(cmd);
            adap.Fill(t);
            if (t.Rows.Count > 0)
            {
        ...
        public bool existeConfiguracion(long CodEmpresa,string CodDominio)
        {
            string strSql = string.Empty;
            DataTable t = new DataTable();
            strSql =
    ...
    public enum DOMINIOS_ENVIOCORREO
    {
        ENVIOCORREO_PORDEFECTO = 10,
        ENVIOCORREO_PORQUANTUM = 1010
    }
