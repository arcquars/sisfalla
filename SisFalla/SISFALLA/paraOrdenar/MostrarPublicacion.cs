    ...
    public partial class MostrarPublicacion : BaseForm
    {
        ...
        private bool PrepararCategoria(string num)
        {
            ...
            cmd.CommandText = @"SELECT
                                ...
                                C.TABLA_INTRANET,
                                C.PK_COD_INTRANET,
                                C.CARPETA_AUX || 'www\media\archivos\20' AS NOM_INTRANET
                                FROM
                                P_DEF_SERVIDOR A,
                                P_DEF_CONEXION B,
                                P_DEF_CATPUBLICACION C
                                WHERE
        ...
        private void preparaArchivo()
        {
            if (PrepararCategoria(_idCategoria))
            {
                ...
                _Numerofalla = _regFalla.CodFalla;
                ...
                pubIntra.num = int.Parse(nFalla.Substring(2, 4));

                a.Text = "SUBIR - " + pubIntra.archivo;
                c.Text = "BAJAR - " + pubIntra.archivo;

                RptInformeFallaParametroExtra paramExtra = new RptInformeFallaParametroExtra(_regFalla.CodFalla, 7, PK_D_COD_TIPOINFORME.FINAL);
                paramExtra.ModoVisible = false;
                paramExtra.NombreArchivoExportar = pubIntra.archivo;
        ...
        {
            bool copiado = false;
            string serverDestino = _regServidor.nombreIntranet +  _regFalla.CodFalla.ToString().Substring(0,2)+ rutadestino + nombredestino;
        ...
        private void RegistrarPublicacion()
        {
            ...
            op.RegistrarOperacion(DOMINIOS_OPERACION.CNDC_PUBLICA_INFORME_FINAL, _Numerofalla, 7);
